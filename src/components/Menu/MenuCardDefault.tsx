...
// Extracted constants for easier configuration and readability
const DEFAULT_IMAGE =
  "https://res.cloudinary.com/ducno7lr7/image/upload/v1782997948/qr-menu/6a4660f567afa405ee2fd563/exmmuyg1p4tdul6xj9cj.jpg"
const MAX_FILE_SIZE_BYTES = 12 * 1024 * 1024 // 12MB limit for image uploads

const MenuCardDefault: React.FC<MenuCardDefaultProps> = ({ onDone, section }) => {
  
  // --- Component State ---
  
  // State for image handling (actual file object for upload, and preview URL for UI)
  const [imageFile, setImageFile] = useState<File | null>(null)
  const [imagePreview, setImagePreview] = useState<string>(DEFAULT_IMAGE)

  // State for the item's details (title, pricing, quantity)
  ...
  // State for API calls and UI feedback
  const [uploading, setUploading] = React.useState<boolean>(false);

  // State to track which specific field is currently being edited by the user
  ...
  /**
   * Submits the new menu item data to the backend API.
   * Uses FormData because we are sending an image file alongside text data.
   */
