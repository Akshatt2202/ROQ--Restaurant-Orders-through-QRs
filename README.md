...
A contactless ordering platform for restaurants. Merchants build a digital menu, generate a QR code per table, and guests scan it to browse, order, and pay — no app install, no waiting for a server to take the order.

Built with Next.js 16 (App Router), TypeScript, MongoDB, and Razorpay.
...
* Dashboard with weekly order/revenue/visitor metrics and week-over-week trends
* Order trend charts and most-ordered-item breakdowns (Recharts)
* Full order and transaction history, exportable to PDF

**For guests**

* Scan → menu loads instantly in the browser, scoped to that merchant
* Add to cart, adjust quantities, checkout
* Razorpay-hosted payment with server-side signature verification
...
NEXT\\\_PUBLIC\\\_RAZORPAY\\\_KEY\\\_ID=your\\\_key\\\_id
```
...
│   │   ├── payment/       # razorpay order, verify, webhook, history
...
|`transaction`|Razorpay payment records — `PENDING` / `COMPLETED` / `FAILED`|
...
|`GET`|`/api/payment?mid=`|Create Razorpay order from cart|
...
5. Checkout creates a Razorpay order server-side; payment is verified by signature before the order is marked complete.
6. The transaction feeds the merchant dashboard in near real time.
